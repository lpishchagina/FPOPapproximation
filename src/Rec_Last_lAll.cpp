...
Rec_Last_lAll::~Rec_Last_lAll() { delete rectangle;  csY = NULL; csY2 = NULL; locCosts = NULL; }

//accessory---------------------------------------------------------------------
std::list<pSphere> Rec_Last_lAll::get_spheresBefore() const { return spheresBefore; }
unsigned int Rec_Last_lAll::get_tau() const { return tau; }

//tools-------------------------------------------------------------------------
double Rec_Last_lAll::get_dist(double* pnt1, double* pnt2) {
  double res = 0;
...
bool Rec_Last_lAll::EmptyOfCandidate() { return rectangle -> IsEmptyRect(); }

void Rec_Last_lAll::idCandidate(unsigned int dim, unsigned int t, double** &csy, double** &csy2, double* &loccosts) {
  p = dim;
  tau = t;
  csY = csy;
  csY2 = csy2;
