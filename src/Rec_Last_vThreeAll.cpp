#include "Rec_Last_vThreeAll.h"
#include <Rcpp.h>

using namespace Rcpp;
using namespace std;

Rec_Last_vThreeAll::Rec_Last_vThreeAll(const Rec_Last_vThreeAll & candidate) {
  Dim = candidate.Dim;
  Tau = candidate.Tau;
...
Rec_Last_vThreeAll::~Rec_Last_vThreeAll() { delete Rect;  CumSumData = NULL;  CumSumData2 = NULL;  VectOfCosts = NULL; }

unsigned int Rec_Last_vThreeAll::GetTau()const { return Tau; }
void Rec_Last_vThreeAll::CleanOfCandidate() { CumSumData = NULL;  CumSumData2 = NULL;  VectOfCosts = NULL; }
bool Rec_Last_vThreeAll::EmptyOfCandidate() { return Rect -> IsEmpty_rect(); }

void Rec_Last_vThreeAll::InitialOfCandidate(unsigned int tau, double** &cumsumdata,  double** &cumsumdata2, double* &vectofcosts) {
  Tau = tau;
...
void Rec_Last_vThreeAll::UpdateOfCandidate(unsigned int IndexToLinkOfUpdCand, std::vector<std::list<Rec_Last_vThreeAll>::iterator> &vectlinktocands, unsigned int& RealNbExclus) {
  RealNbExclus = 0;
  Cost cost = Cost(Dim);
